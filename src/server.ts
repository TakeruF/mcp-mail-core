...
export function buildMailServer(service: MultiAccountMailService): McpServer {
  ...
  register("reply_mail", "Reply from the account that owns the selected message. Requires confirm=true.", z.object({ ref: messageRef, message: outgoing.omit({ to: true, subject: true }), confirm }).strict(), write, (input) => service.reply(compact(input) as Parameters<typeof service.reply>[0]));
